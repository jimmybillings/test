...
import {DIST_DIR, DEV_DEST, PROD_DEST, TMP_DIR, LIBRARY_DEST} from '../config';
...
export = function clean(gulp, plugins, option) {
  ...
  return function(done) {
    switch(option) {
      ...
      case 'library' : cleanLibrary(done); break;
      default: done();
    }
  };
...
function cleanLibrary(done) {
  del([LIBRARY_DEST, TMP_DIR]).then((paths) => {
    util.log('Deleted', chalk.yellow(paths && paths.join(', ') || '-'));
    done();
  });
}
